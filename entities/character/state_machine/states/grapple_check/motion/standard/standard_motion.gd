...
func update(delta) -> void:
	...
	temp_velocity.y += _gravity * _gravity_multiplier * delta
	temp_velocity.x = lerp(_velocity.x, _move_speed * get_input_direction().x, _move_acceleration)

	if !_has_grapple_length(temp_velocity):
		emit_signal("finished", _get_grapple_equivalent())
		return
